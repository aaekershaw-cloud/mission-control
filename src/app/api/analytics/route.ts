...
export async function GET() {
  try {
    ...
    const queueHistory = await db.all(`
      ...
      GROUP BY DATE(created_at)
      ORDER BY day ASC
    `, []) as Array<{ day: string; count: number; completed: number; errors: number }>;

    // Activity timeline (last 24 hours)
    const timelineRows = await db.all(`
      SELECT TO_CHAR(DATE_TRUNC('hour', created_at), 'YYYY-MM-DD"T"HH24:00:00') AS hour, 'task' AS source
      FROM tasks WHERE created_at >= NOW() - INTERVAL '24 hours'
      UNION ALL
      SELECT TO_CHAR(DATE_TRUNC('hour', created_at), 'YYYY-MM-DD"T"HH24:00:00') AS hour, 'message' AS source
      FROM messages WHERE created_at >= NOW() - INTERVAL '24 hours'
    `, []) as Array<{ hour: string; source: string }>;
